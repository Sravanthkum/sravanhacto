...
for i, month in enumerate(months):
    ...
    # Paid salary value
    paid_l = Label(root, text="0.00", fg="black", width=10, anchor="e", relief="ridge")
...
lbl_total_salary = Label(root, text="", fg="black", width=20, anchor="w")
lbl_total_salary.place(x=230, y=730)
Label(root, text="Total Number Of Leaves:", width=25, anchor="w").place(x=400, y=730)
lbl_total_leaves = Label(root, text="", fg="black", width=20, anchor="w")
lbl_total_leaves.place(x=600, y=730)
Label(root, text="Total PT Amount Debited:", width=25, anchor="w").place(x=700, y=730)
lbl_total_pt = Label(root, text="", fg="black", width=20, anchor="w")
lbl_total_pt.place(x=900, y=730)
Label(root, text="Total Paid Salary:", width=25, anchor="w").place(x=1100, y=730)
lbl_total_paid = Label(root, text="", fg="black", width=20, anchor="w")
lbl_total_paid.place(x=1230, y=730)

# TDS Label
Label(root, text="TDS Amount Deducted:", width=25, anchor="w").place(x=30, y=760)
lbl_tds = Label(root, text="", fg="black", width=20, anchor="w")
lbl_tds.place(x=230, y=760)
...
def calculate():
    ...
    for i in range(12):
        try:
            ...
            sal = 0
        try:
            pt = float(pt_entries[i].get())  # Fixed typo here
        except:
            pt = 0
        try:
    ...
    lbl_tds.config(text=f"{tds:,.2f}")

Button(root, text="submit", command=calculate, bg="white", fg="black", width=15).place(x=650, y=630)
root.mainloop()
